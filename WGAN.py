...
BATCH_SIZE = 4    
# The batch size is better to be 128 for getting the best accuracy according WGAN article. 
# The GPU memory is a limitation to set the batch size to 128. I will use Gradient 
# accumulation method to run the mini-batches sequentially, while accumulating the 
# gradients, in next project. Then, The accumulated results are used to update the 
# model variables at the end of the last mini-batch. The related code will be posted
# soon. 
...
def generator_loss(disc_generated_output, gen_output, target):
    ...
    total_gen_loss = gan_loss + (LAMBDA2 * l2_loss)
...
def gradient_penalty(inp, gen_image):
...
def train_step(images, target, step):
    ...
    with tf.GradientTape() as gen_tape:
        ...
        gen_total_loss, gen_gan_loss, gen_l2_loss = generator_loss(fake_output, generated_images, target)
    ...
    with summary_writer.as_default():
        ...
        tf.summary.scalar('gp', gp, step=step//1000)
