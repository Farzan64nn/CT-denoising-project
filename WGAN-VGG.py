...
OUTPUT_CHANNELS = 1
BUFFER_SIZE = 482075
IMAGE_SHAPE = (IMG_HEIHGT , IMG_WIDTH , OUTPUT_CHANNELS)
BATCH_SIZE = 2
...
def generator_loss(disc_generated_output, gen_output, target):
    ...
    total_gen_loss =  gan_loss + (LAMBDA1 * l2_loss) + (LAMBDA3 * feature_loss)
    return total_gen_loss, gan_loss, feature_loss
...
def train_step(images, target, step):
    ...
    with tf.GradientTape() as gen_tape:
        ...
        gen_total_loss, gen_gan_loss, feature_loss = generator_loss(fake_output, generated_images, target)
